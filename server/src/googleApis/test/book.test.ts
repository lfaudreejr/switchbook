...
describe('google Books Api', () => {
  describe('getBook()', () => {
    ...
    it('return status 200', (done) => {
      chai.expect(book).to.eventually.haveOwnProperty('status').that.equals(200).and.notify(done);
    });
    it('returns items array', (done) => {
      chai.expect(book).to.eventually.haveOwnProperty('data').that.haveOwnProperty('items').and.notify(done);
    });
    it('returns book details', (done) => {
      chai
        .expect(book)
        .to.eventually.haveOwnProperty('data')
        .that.haveOwnProperty('items')
